...
const state = {
  ...
  chatMessages:      [],
  rematchId:         null,
  rematchStatus:     null, // pending | accepted | declined | incoming
};
...
const api = {
  ...
  sendChat:       (gid, pid, message)          => api.request("POST",   `/games/${gid}/chat`,          { player_id: pid, message }),
...
function renderGamesList() {
  ...
  container.querySelectorAll("button[data-game-id]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (action === "join") joinGame(gid);
      else if (action === "spectate") enterSpectator(gid);
      else enterGame(gid);
    });
  });
...
async function handleCreateGame() {
  ...
  try {
    const game = await api.createGame(gs, mp, state.player.player_id);
...
async function refreshGame() {
  ...
  try {
    ...
    // Poll chat if game is active or placing
    if (game.status === "active" || game.status === "placing") {
      pollChat();
    }
    ...
    // Poll rematch if game is finished
...
function renderSpectatorView(game) {
  ...
  statusEl.className   = `status-pill status-${game.status}`;
  statusEl.textContent = game.status.replace("_", " ");

  const turnEl = document.getElementById("turn-label");
  turnEl.classList.remove("my-turn");
  if (game.status === "active") {
    turnEl.textContent = `${getUsername(game.current_turn_player_id)}'s turn`;
  } else {
    turnEl.textContent = game.status === "finished" ? "Game over" : game.status;
  }

  // Show all shots on both boards
  ...
  // Left board = player 1
  const boardEl1 = document.getElementById("my-board");
  const cells1   = makeBoardGrid(boardEl1, game.grid_size);
  document.querySelector(".board-wrap h3").textContent = getUsername(p1.player_id);

  // Right board = player 2
  const boardEl2 = document.getElementById("opp-board");
  const cells2   = makeBoardGrid(boardEl2, game.grid_size);
  document.getElementById("opp-title").textContent = getUsername(p2.player_id);
  document.getElementById("opp-tabs").innerHTML = "";

  // Paint shots
  state.moves.forEach(m => {
    // Shots by p2 show on p1's board
    if (m.player_id === p2.player_id) {
      if (cells1[m.row]?.[m.col]) cells1[m.row][m.col].classList.add(m.result === "hit" ? "hit" : "miss");
    }
    // Shots by p1 show on p2's board
    if (m.player_id === p1.player_id) {
      if (cells2[m.row]?.[m.col]) cells2[m.row][m.col].classList.add(m.result === "hit" ? "hit" : "miss");
    }
  });
...
function renderGameView() {
  ...
  if (g.status === "active") {
    if (g.current_turn_player_id === state.player.player_id) {
      turnEl.textContent = "🎯 Your turn"; turnEl.classList.add("my-turn");
    } else {
    ...
    turnEl.textContent = `Waiting (${(g.player_ids||[]).length}/${g.max_players})`;
  }
...
function renderPhaseBanner(g) {
  ...
  if (g.status === "waiting_setup") {
    ...
    if ((g.player_ids || []).length >= 2) {
      banner.innerHTML = `Waiting for more players (or click to start). <button id="banner-start" class="primary small" style="margin-left:12px">Start Game</button>`;
      banner.classList.remove("hidden");
      const btn = document.getElementById("banner-start");
      if (btn) btn.onclick = handleStartGame;
    } else {
      banner.textContent = `Waiting for ${need} more player${need !== 1 ? "s" : ""}...`;
      banner.classList.remove("hidden");
    }
  } else if (g.status === "placing") {
    if (state.myShips.length === 4) {
      banner.textContent = "All ships placed. Waiting for opponent...";
      ...
      if (xBtn) xBtn.onclick = () => { state.placementShips = []; renderGameView(); };
    } else {
      const rem = 4 - state.placementShips.length;
      banner.textContent = `Place ${rem} more ship${rem !== 1 ? "s" : ""}. Click your board.`;
    }
    banner.classList.remove("hidden");
  } else {
...
function getShipCells(type, startRow, startCol, orientation) {
  ...
  for (let i = 0; i < length; i++) {
    cells.push(orientation === "H" ? [startRow, startCol + i] : [startRow + i, startCol]);
  }
  return cells;
}
...
function handlePlacementClick(row, col, gridSize, cells) {
  const type = nextShipToBePlaced();
  if (!type || state.placementShips.length >= 4) return;
  if (!isValidPlacement(type, row, col, state.orientation, gridSize, state.placementShips)) {
    toast("Can't place there — out of bounds or overlapping.", "error"); return;
  }
  ...
  selector.innerHTML = `
    ...
    <div class="ship-tray">
      ${SHIP_DEFS.map(def => {
        ...
        return `
          <div class="ship-option ${isSelected ? "selected" : ""} ${isPlaced ? "placed" : ""}" data-type="${def.type}">
            ...
            ${isPlaced
              ...
              : `<button class="rotate-btn" data-type="${def.type}" title="Rotate">↺</button><span class="orientation-badge">${isSelected ? state.orientation : "H"}</span>`}
          </div>`;
      }).join("")}
    </div>`;
...
function renderOpponentBoard(g) {
  ...
  if (opponents.length > 1) {
    ...
    tabsEl.querySelectorAll("button[data-pid]").forEach(btn => {
      btn.addEventListener("click", () => { state.selectedOpponent = parseInt(btn.dataset.pid, 10); renderGameView(); });
    });
...
async function pollChat() {
  ...
  try {
    const msgs = await api.getChat(state.currentGame.id, state.player.player_id);
...
async function handleSendChat() {
  ...
  input.value = "";
  try {
    await api.sendChat(state.currentGame.id, state.player.player_id, message);
    await pollChat();
  } catch (e) { toast(e.message, "error"); }
}
...
async function pollRematch() {
  ...
  try {
    ...
    state.rematchId = rematch.id;
    const myId      = state.player.player_id;

    if (rematch.status === "accepted" && state.rematchStatus !== "accepted") {
      state.rematchStatus = "accepted";
      showRematchView("accepted");
      setTimeout(() => {
        document.getElementById("win-modal").classList.add("hidden");
        enterGame(rematch.new_game_id);
      }, 2000);
      return;
    }

    if (rematch.status === "declined" && state.rematchStatus !== "declined") {
      state.rematchStatus = "declined";
      showRematchView("declined");
      return;
    }

    if (rematch.status === "pending") {
      if (rematch.opponent_id === myId && state.rematchStatus !== "incoming") {
        state.rematchStatus = "incoming";
        showRematchView("incoming");
      } else if (rematch.requester_id === myId && state.rematchStatus !== "pending") {
        state.rematchStatus = "pending";
        showRematchView("pending");
      }
    }
  } catch (_) {}
}
...
function showRematchView(view) {
  ["requester", "pending", "incoming", "declined", "accepted"].forEach(v => {
    const el = document.getElementById(`rematch-${v}-view`);
    if (el) el.classList.add("hidden");
  });
  const target = document.getElementById(`rematch-${view}-view`);
...
function showPostGameModal(g) {
  ...
  document.getElementById("postgame-this-game").innerHTML = statGrid([
    { val: gameShots, label: "Shots" }, { val: gameHits, label: "Hits" },
    { val: gameMisses, label: "Misses" }, { val: `${gameAcc}%`, label: "Accuracy" },
  ]);
  ...
  document.getElementById("postgame-lifetime").innerHTML = statGrid([
    { val: p.wins || 0, label: "Wins" }, { val: p.losses || 0, label: "Losses" },
    { val: p.total_shots || 0, label: "Shots" }, { val: `${lifeAcc}%`, label: "Accuracy" },
  ]);

  // Reset rematch UI
  showRematchView("requester");
  state.rematchStatus = null;
...
async function showLeaderboard() {
  ...
  try {
    ...
    body.querySelectorAll(".lb-row[data-pid]").forEach(row => {
      row.addEventListener("click", () => showProfile(parseInt(row.dataset.pid, 10)));
    });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("btn-win-close").addEventListener("click", () => {
    document.getElementById("win-modal").classList.add("hidden");
    state.currentGame = null; state.hitStreak = 0;
    showView("lobby");
  });
  ...
  document.getElementById("btn-rematch-decline").addEventListener("click", async () => {
    if (!state.rematchId) return;
    try {
      await api.respondRematch(state.rematchId, state.player.player_id, "decline");
      state.rematchStatus = "declined";
      showRematchView("declined");
    } catch (e) { toast(e.message, "error"); }
  });
